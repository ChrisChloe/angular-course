...
class HirePlanDialog extends React.Component {
  ...
  setInitialValues = () => {
    const { card, infoPlan } = this.props
    getSessionId()
    window.PagSeguroDirectPayment.setSessionId(window.localStorage.sessionId)

    if (card.brand === undefined) {
      initialValues.plan_id = infoPlan.id || ''
      this.setState({ openForm: true })
    } else {
      initialValues.plan_id = infoPlan.id || ''
      initialValues.brand = card.brand || ''
      initialValues.cardNumber = card.number.replace(/ /g, '') || ''
      initialValues.expirationMonth = card.expiration_month || ''
      initialValues.expirationYear = card.expiration_year || ''
      initialValues.owner = card.holder.name || ''
      initialValues.phone = card.holder.phone || ''
      this.setState({ openForm: true })
    }
  }
  ...
  render() {
    const { open, handleCloseModal, infoPlan } = this.props
    const {
      openSnack, openForm, message, paymentInfo,
      loading,
    } = this.state
      ...
      <div>
        ...
        >
          ...
          <DialogContent>
            ...
            {infoPlan.features.map(feature => (
              <DialogContentText id="alert-dialog-description" key={feature}>
                {'- '}
                { feature }
              </DialogContentText>
            ))}
                          ...
                          <form onSubmit={handleSubmit} noValidate>
                            <Grid container spacing={24}>
                              ...
                              <Grid item xs={12} style={{ textAlign: 'right' }}>
                                <Button onClick={this.handleCloseForm} style={{ margin: '0 10px' }}>
                                  Cancelar
                                </Button>
                                <Button
                                  variant="contained"
          ...
          { openForm === false ? (
            <DialogActions>
              ...
              <Button onClick={this.setInitialValues} variant="contained" color="primary" autoFocus>
                    Contratar
              </Button>
            </DialogActions>
          ) : null }
